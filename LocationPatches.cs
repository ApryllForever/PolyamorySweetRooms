              ...
              //  {
               ...
               // var allSpouses = allSpises;
                ...
                var allSpouses = GetSpouses(__instance.owner, -1).Keys.ToList();
                 ...
                 //Prevents the enter house crash bug
              ...
              //  }
               ...
               GetFarmHouseSpouseRooms(__instance, allSpouses, out List<string> orderedSpouses, out List<string> customSpouses);
                ...
                for (int i = 0; i < orderedSpouses.Count; i++)
                {
                    ...
                    if (SHelper.ModRegistry.Get("crystalinerose.RusticFarmhouse") != null)
                    {
                        if (i == 0 && __instance.upgradeLevel == 1)
                        {
                            srd.shellType += "_KisaRustic";
                        }
                        else if (i == 0 && __instance.upgradeLevel > 1)
                        {
                            srd.shellType += "_KisaRustic";
                        }
                    }

                    if (SHelper.ModRegistry.Get("Aimon111.AimonsTidyCozyFarmhouse") != null)
                    {
                        if (i == 0 && __instance.upgradeLevel == 1)
                        {
                            srd.shellType += "_ATidCoz";
                        }
                        else if (i == 0 && __instance.upgradeLevel > 1)
                        {
                            srd.shellType += "_ATidCoz";
                    ...
                    if (i == 0 && __instance.upgradeLevel > 1 && Game1.player.mailReceived.Contains("renovation_corner_open") && SHelper.ModRegistry.Get("Aimon111.AimonsTidyCozyFarmhouse") == null && SHelper.ModRegistry.Get("crystalinerose.RusticFarmhouse") == null)
                    {
                        srd.shellType += "_r";
                    }
        ...
        private static void MakeSpouseRoom(DecoratableLocation location, HashSet<string> appliedMapOverrides, SpouseRoomData srd, bool first = false)
        {
            try
            {
                if (srd != null)
                {
                    ...
                    //
                    // This right here below is what causes the crash. Extending the layers kills things. To rememdy, I have made a CP patch which hard enlarges the map a lot.  
                    //
                    //{
